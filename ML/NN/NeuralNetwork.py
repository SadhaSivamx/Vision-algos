...
for _ in range(epochs):
    ...
    for idx in range(len(datapointsx)):
        ...
        #Backward-Prop
        dw3=-2*(ytrue-out3)*reludr(z3)*out2
        dw2=np.matmul((np.matmul(-2*(ytrue-out3)*reludr(z3),a3)*reludr(z2)).T,out1)
        dw1=np.matmul((np.matmul(-2*(ytrue-out3)*reludr(z3)*a3*reludr(z2),a2)*reludr(z1)).T,x)
